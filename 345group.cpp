...
int main() {
	...
	levelTitle.setPosition(240, 250);
	...
	strLevel.setPosition(240, 300);

	//main playing area container
	//sf::RectangleShape container(sf::Vector2f(330, 600));
	sf::RectangleShape container(sf::Vector2f(300, 600));
	...
	while (window.isOpen())
	{
		...
		clock.restart();
		timer += time; // update timer with last timer value, combined into current timer value
